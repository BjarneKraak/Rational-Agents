...
As suggested to try, it is clear that in a scenario with many enemies, the chances of the robot to survive decreases. An similar algorithm used for finding a powerstation in case of less energy could be implemented to find a friend when the robot gets too damaged. In That way, the robots' chances for surviving in an environment with many enemies increases. 
@#$#@#$#@
default
true
0
