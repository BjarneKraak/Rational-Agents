...
to setup-turtles
  ...
  create-robots 1    [set color grey                         ;; there is only one robot needed, which is twice the size of the other agents
                      ...
                      set energy 10              ;; the initial energy is not enough to reach the goal and set by a slider
                      ]
...
to setup-patches
  ...
  ask n-of 10 patches [set pcolor red]               ;; slider sets amount of obstacles randomly distributed with red color
  ask n-of 10 patches [set pcolor green]         ;; slider sets amount of power stations randomly distributed with green color
...
to move-robots
  ask robots [                             ;; enemies move in a random direction either 1,2 or 3 steps forward
    right random 380
    forward one-of [1 5 8 4 13 7]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
